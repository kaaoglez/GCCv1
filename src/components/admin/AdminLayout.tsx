...
export function AdminLayout({ children, onBack }: { children: React.ReactNode; onBack?: () => void }) {
  const { activePage, setActivePage, logout, adminUser } = useAdminStore();
  ...
  const userName = adminUser?.name || 'Administrador';
    ...
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: '#f0f0eb' }}>
      ...
      >
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="hidden sm:inline text-xs" style={{ color: '#888' }}>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={handleBack}
            className="hidden sm:flex items-center gap-1.5 px-2 py-1 rounded hover:bg-white/10"
            title="Ver sitio"
          >
          ...
          <div className="flex items-center gap-2 px-2 py-1 rounded hover:bg-white/10 cursor-pointer">
            <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold" style={{ backgroundColor: '#1B4332', color: '#fff' }}>
              {userName.charAt(0).toUpperCase()}
            </div>
            <span className="hidden sm:inline font-medium text-sm" style={{ color: '#eee' }}>
              {userName}
            </span>
          </div>
          <button
      ...
      <div className="flex flex-1 pt-11">
        ...
        <div className="flex-1 lg:ml-60 flex flex-col min-h-0">
          ...
          <div
            className="sticky top-11 z-20 border-b bg-white/80"
            style={{ borderColor: '#d4ddd4', backdropFilter: 'blur(8px)' }}
          >
            <div className="flex items-center justify-between px-5 lg:px-6 h-14">
              ...
              <Badge variant="outline" className="text-xs font-normal" style={{ borderColor: '#d4ddd4', color: '#666' }}>
                {userRole}
              </Badge>
            </div>
          </div>
